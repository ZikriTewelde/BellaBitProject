...
-- LOOKING AT HOW MANY USERS WE HAVE IN THE TOTAL STEPS DATA
...
SELECT ACT.Id, ACT.ActivityDate, SLP.TotalMinutesAsleep,(ACT.VeryActiveMinutes+ACT.LightlyActiveMinutes+ACT.FairlyActiveMinutes) AS TotalActiveTime, (SLP.TotalTimeInBed-SLP.TotalMinutesAsleep) AS TimeToFallAsleep
FROM BellaBit_CaseStudy..dailyActivity_mergedCSV ACT
JOIN BellaBit_CaseStudy..sleepDay_mergedCSV SLP
   ON ACT.Id = SLP.Id
   AND ACT.ActivityDate = SLP.SleepDay
...
SELECT STP.Id,STP.ActivityDay, STP.StepTotal, SLP.TotalMinutesAsleep,(SLP.TotalTimeInBed-SLP.TotalMinutesAsleep) AS QualityOfSleep
...
--looking at sleep duration and calories burnt

SELECT ACT.Id, ACT.ActivityDate, SLP.TotalMinutesAsleep, (SLP.TotalTimeInBed-SLP.TotalMinutesAsleep) AS TimeToFallAsleep, ACT.Calories
FROM BellaBit_CaseStudy..dailyActivity_mergedCSV ACT
JOIN BellaBit_CaseStudy..sleepDay_mergedCSV SLP
   ON ACT.Id = SLP.Id
   AND ACT.ActivityDate = SLP.SleepDay
GROUP BY ACT.Id, ACT.ActivityDate,SLP.TotalMinutesAsleep,TotalTimeInBed,ACT.LightlyActiveMinutes, ACT.FairlyActiveMinutes, ACT.VeryActiveMinutes,ACT.Calories
...
-- LOOKING FOR ANY REALATION BETWEEN THE AMOUNT OF CALORIES BURNT BY USERS AND THE DIFFERENT OTHER DIMENTIONS OF THE DATA

USE BellaBit_CaseStudy
SET ANSI_NULLS ON 
SET QUOTED_IDENTIFIER ON
CREATE VIEW CaloriesAndDistance AS(
SELECT Id, TotalDistance, Calories
FROM BellaBit_CaseStudy..dailyActivity_mergedCSV
GROUP BY Id,TotalDistance,Calories
...
SET ANSI_NULLS ON 
SET QUOTED_IDENTIFIER ON
CREATE VIEW CaloriesAndActivity AS(
SELECT Id,VeryActiveMinutes, FairlyActiveMinutes,LightlyActiveMinutes,(VeryActiveMinutes+FairlyActiveMinutes+LightlyActiveMinutes) AS TotalActiveMinutes, Calories
FROM BellaBit_CaseStudy..dailyActivity_mergedCSV
GROUP BY Id,Calories,VeryActiveMinutes, FairlyActiveMinutes,LightlyActiveMinutes
